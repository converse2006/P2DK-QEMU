# 1 "/home/converse/virtual_platform1/testsuit/hybrid/db_tmpl/pac.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/home/converse/virtual_platform1/testsuit/hybrid/db_tmpl/pac.c"
...
{pp_mbox_send_w(msg);}
# 4 "/home/converse/virtual_platform1/testsuit/hybrid/db_tmpl/pac.c" 2
# 1 "/home/converse/virtual_platform1/testsuit/hybrid/db_tmpl/mem.h" 1
# 5 "/home/converse/virtual_platform1/testsuit/hybrid/db_tmpl/pac.c" 2
...
# 6 "/home/converse/virtual_platform1/testsuit/hybrid/db_tmpl/pac.c" 2

uint8_t __attribute__((section(".ldm_shared"), aligned(4))) inbuffer0[4096];
uint8_t __attribute__((section(".ldm_shared"), aligned(4))) inbuffer1[4096];
uint8_t __attribute__((section(".ldm_shared"), aligned(4))) outbuffer0[4096];
uint8_t __attribute__((section(".ldm_shared"), aligned(4))) outbuffer1[4096];
# 38 "/home/converse/virtual_platform1/testsuit/hybrid/db_tmpl/pac.c"
int main(int argc, char *argv[])
{
    ...
    do { puts("\nHello from pacdsp\n");} while(0);
    do {
        do { puts("Waitling for new task....\n");} while(0);
        do { do { puts("# recv ");} while(0); pp_wait_mbox(&msg); do { puts("cmd: 0x");} while(0); do { puth((&msg)->cmd);} while(0); do { puts(", data: 0x");} while(0); do { puth((&msg)->data);} while(0); do { putc('\n');} while(0); } while(0);;

        if(msg.cmd == 0x00000007) {
            do { puts("All task done, exit\n");} while(0);
            return 0;
        } else if(msg.cmd != 0x00000001) {
            do { puts("Error: "); puts("Protocal error"); puts(", exist\n"); return -1; } while(0);
        } else {
            do { puts("new task started!\n");} while(0);
        ...
        do {
            ...
            if(msg.cmd == 0x00000006) {
                task_end(&msg.data);
                do { do { puts("# send cmd: 0x");} while(0); do { puth((&msg)->cmd);} while(0); do { puts(", data: 0x");} while(0); do { puth((&msg)->data);} while(0); pp_send_mbox(&msg); do { puts(".\n");} while(0); } while(0);;
                break;
